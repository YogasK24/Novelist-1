        ...
        @switch (activeTab()) {
            @case ('characters') {
                @if (bookState.isLoadingChildren().characters) {
                     <p class="text-gray-500 text-sm text-center py-4">Loading characters...</p>
                    ...
                    @if (characters.length > 0) {
                        ...
                        <p class="text-gray-500 text-sm text-center py-4">No characters added yet.</p>
                    }
                }
            }
            @case ('locations') {
                @if (bookState.isLoadingChildren().locations) {
                     <p class="text-gray-500 text-sm text-center py-4">Loading locations...</p>
                    ...
                    @if (locations.length > 0) {
                        <div class="space-y-2">
                         ...
                         <p class="text-gray-500 text-sm text-center py-4">No locations added yet.</p>
                    }
                }
            }
            @case ('events') {
                @if (bookState.isLoadingChildren().plotEvents) {
                     <p class="text-gray-500 text-sm text-center py-4">Loading events...</p>
                     ...
                     @if (events.length > 0) {
                        ...
                        <p class="text-gray-500 text-sm text-center py-4">No events added yet.</p>
                     }
                }
            }
        }
