      ...
      @if (bookState.isLoading() === 'loading') {
        <div class="flex justify-center items-center py-6">
         ...
         @if (props.length > 0) {
            ...
            <div class="space-y-3">
              @for (prop of props; track prop.id) {
                <div class="bg-gray-800 p-4 rounded-lg shadow flex justify-between items-start">
                  <!-- Info Properti -->
                  <div class="mr-4">
                     <h3 class="text-lg font-semibold text-white">{{ prop.name }}</h3>
                     <p class="text-sm text-gray-400 mt-1 whitespace-pre-wrap">{{ prop.description || 'Tidak ada deskripsi.' }}</p>
                  </div>
                  <!-- Tombol Aksi (Edit/Hapus) -->
                  <div class="flex-shrink-0 space-x-2">
                     <button (click)="openEditModal(prop)" class="text-blue-400 hover:text-blue-300 p-1" aria-label="Edit Properti">
                     ...
                     <button (click)="deleteProp(prop.id!, prop.name)" class="text-red-400 hover:text-red-300 p-1" aria-label="Hapus Properti">
           ...
           <p class="text-center text-gray-500 py-6">Belum ada properti/item. Klik tombol di atas untuk menambah!</p>
         }
      }
