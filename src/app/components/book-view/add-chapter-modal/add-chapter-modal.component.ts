    ...
    <div 
      ...
      <div 
        ...
        <form [formGroup]="chapterForm" (ngSubmit)="onSubmit()">
          <div class="mb-4">
            <label for="chapterTitle" class="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-1">Judul Bab</label>
            <input
              type="text"
              id="chapterTitle"
          ...
          <div class="mb-6">
             <label class="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Karakter yang Muncul</label>
             <div class="bg-slate-100 dark:bg-slate-700 p-3 rounded-md max-h-32 overflow-y-auto">
               @for (char of bookState.characters(); track char.id) {
                 <div class="flex items-center mb-1">
                   ...
                   <label [for]="'chap-char-' + char.id" class="ml-2 text-sm text-slate-700 dark:text-slate-300">{{ char.name }}</label>
                 </div>
               }
