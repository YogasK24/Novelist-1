        ...
        <div class="flex justify-center items-center py-6"> 
         ...
         @if (plotEvents.length > 0) {
            ...
            <div 
              ...
              class="relative pl-8 before:content-[''] before:absolute before:top-0 before:left-4 before:bottom-0 before:w-0.5 before:bg-gray-300 dark:before:bg-gray-700">
              ...
              @for (event of plotEvents; track event.id) {
                ...
                <div 
                  ...
                  <div class="absolute -left-[calc(1rem+2px)] top-1 w-4 h-4 rounded-full bg-purple-500 dark:bg-purple-600 border-2 border-white dark:border-gray-800 group-hover:bg-purple-400 transition"></div>
                  
                  <!-- Timeline Content (card) -->
                  <div 
                    (click)="viewPlotEventDetails(event)"
                    class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md hover:shadow-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/80 transition-all duration-200 cursor-pointer">
                    
                    <div class="flex justify-between items-start">
                        <div class="flex-grow">
                            <h4 class="font-bold text-gray-900 dark:text-white text-lg">{{ event.title }}</h4>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-2 whitespace-pre-wrap break-words">
                     ...
                     <!-- Tags untuk relasi -->
                    <div class="flex items-center flex-wrap gap-x-4 gap-y-1 mt-3 pt-3 border-t border-gray-200/50 dark:border-gray-700/50 text-xs text-gray-600 dark:text-gray-400">
                       @if (event.locationId) {
                         <div class="flex items-center gap-1.5" title="Lokasi">
                           <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 dark:text-gray-500" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" /></svg>
                           <span>1 Lokasi</span>
                         </div>
                       }
                       @if (event.characterIds && event.characterIds.length > 0) {
                           <div class="flex items-center gap-1.5" title="Karakter">
                             <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400 dark:text-gray-500" viewBox="0 0 20 20" fill="currentColor"><path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z" /></svg>
                    ...
                    <div class="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <button [disabled]="isReordering()" (click)="openEditModal(event); $event.stopPropagation()" class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 p-1.5 bg-gray-100/60 dark:bg-gray-900/60 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Edit Event">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                        </button>
                        <button [disabled]="isReordering()" (click)="deletePlotEvent(event.id!, event.title); $event.stopPropagation()" class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 p-1.5 bg-gray-100/60 dark:bg-gray-900/60 rounded-full focus:outline-none focus:ring-2 focus:ring-red-500" aria-label="Hapus Event">
