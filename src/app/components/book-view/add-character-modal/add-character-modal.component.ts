...
export class AddCharacterModalComponent {
  ...
  async onSubmit(): Promise<void> {
    ...
    const { name, description } = this.characterForm.value;
    const character = this.characterToEdit();

    try {
      if (character && character.id) {
        await this.bookState.updateCharacter(character.id, { name: name!, description: description! });
      } else {
        await this.bookState.addCharacter(name!, description!);
      }
      this.close();
    } catch (error) {
