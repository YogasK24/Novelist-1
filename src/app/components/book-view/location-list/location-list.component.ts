      ...
      @if (bookState.isLoading$ | async) {
        <div class="flex justify-center items-center py-6">
         ...
         @if (locations.length > 0) {
              ...
              @for (loc of locations; track loc.id) {
                <div class="bg-gray-800 p-4 rounded-lg shadow flex justify-between items-start">
                  ...
                  <div class="mr-4">
                     ...
                     <p class="text-sm text-gray-400 mt-1 whitespace-pre-wrap">{{ loc.description || 'Tidak ada deskripsi.' }}</p>
                  </div>
                  <!-- Tombol Aksi (Edit/Hapus) -->
                  <div class="flex-shrink-0 space-x-2">
                     <button (click)="openEditModal(loc)" class="text-blue-400 hover:text-blue-300 p-1" aria-label="Edit Lokasi">
                     ...
                     <button (click)="deleteLocation(loc.id!, loc.name)" class="text-red-400 hover:text-red-300 p-1" aria-label="Hapus Lokasi">
           ...
           <p class="text-center text-gray-500 py-6">Belum ada lokasi. Klik tombol di atas untuk menambah!</p>
         }
      }
