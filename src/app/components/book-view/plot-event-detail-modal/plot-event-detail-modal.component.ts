      ...
      <div 
        ...
        @if (event(); as currentEvent) {
          ...
          <div class="max-h-[70vh] overflow-y-auto pr-2">
              <p class="text-slate-700 dark:text-slate-300 whitespace-pre-wrap mb-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Lokasi -->
                <div>
                    <h3 class="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Lokasi</h3>
                    @if (getLocationName(currentEvent.locationId); as locationName) {
                        <div class="flex items-center gap-2 bg-slate-100 dark:bg-slate-700/50 p-3 rounded-md">
                             <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-500 dark:text-slate-400" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
                             </svg>
                            <span class="text-slate-900 dark:text-white font-medium">{{ locationName }}</span>
                        </div>
                    } @else {
                        <p class="text-slate-500 dark:text-slate-400 italic">Tidak ada lokasi spesifik.</p>
                    }
                </div>
                
                <!-- Karakter Terlibat -->
                <div>
                    <h3 class="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Karakter Terlibat</h3>
                    @if (getCharacterNames(currentEvent.characterIds); as charNames) {
                        @if (charNames.length > 0) {
                            <div class="space-y-2">
                                @for (name of charNames; track name) {
                                    <div class="flex items-center gap-2 bg-slate-100 dark:bg-slate-700/50 p-2 rounded-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-500 dark:text-slate-400" viewBox="0 0 20 20" fill="currentColor">
                             ...
                             <p class="text-slate-500 dark:text-slate-400 italic">Tidak ada karakter yang terlibat.</p>
                        }
                    }
                </div>
              </div>
