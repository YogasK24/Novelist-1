            ...
            <div class="space-y-3">
              @for (char of characters; track char.id) {
                <div (click)="viewCharacterDetails(char)"
                     class="bg-gray-800 p-4 rounded-lg shadow flex justify-between items-start cursor-pointer hover:bg-gray-700/80 transition duration-150">
                  <div class="mr-4 flex-grow">
                     <h3 class="text-lg font-semibold text-white">{{ char.name }}</h3>
                     <p class="text-sm text-gray-400 mt-1 whitespace-pre-wrap line-clamp-2">{{ char.description || 'Tidak ada deskripsi.' }}</p>
                  ...
                  <div class="flex-shrink-0 space-x-2 flex items-center">
                     <button (click)="openEditModal(char); $event.stopPropagation()" class="text-blue-400 hover:text-blue-300 p-1" aria-label="Edit Karakter">
                     ...
                     <button (click)="deleteCharacter(char.id!, char.name); $event.stopPropagation()" class="text-red-400 hover:text-red-300 p-1" aria-label="Hapus Karakter">
