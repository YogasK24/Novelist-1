        ...
        <div class="flex justify-center items-center py-6"> 
         ...
         @if (chapters.length > 0) {
           ...
           @if (bookState.contextualSearchTerm()) {
             ...
             <p class="text-center text-gray-500 dark:text-gray-500 py-6">No chapters yet. Click the button above to start your manuscript!</p>
           }
         }
      }
...
export class ChapterListComponent {
  ...
  onMoveItem(chapter: IChapter, event: KeyboardEvent): void {
      ...
      if (newIndex !== -1) {
          ...
          // 1. Simulate drop event (only need currentIndex and previousIndex)
          ...
          // 2. Keep focus on the newly moved item
          setTimeout(() => {
  ...
  onDrop(event: CdkDragDrop<IChapter[]>): void {
    ...
    // Important: Use the original list for sorting, not the filtered list
    const currentChapters = [...this.bookState.chapters()];
    
    // Find the actual item in the original list by ID
    ...
    // Move the item from the old position
    currentChapters.splice(previousIndexInFull, 1);
    
    // Determine the new position in the original list
    // If moving down, the target index might be smaller
