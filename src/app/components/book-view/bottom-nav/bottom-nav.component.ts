...
@Component({
  ...
  template: `
    @if (bookState.currentBookId(); as bookId) {
      <nav class="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm border-t border-gray-200 dark:border-white/10 shadow-inner z-40">
        <div class="container mx-auto flex justify-around items-center h-16 text-sm">
           ...
           <a [routerLink]="['/book', bookId]" 
              ...
              class="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition px-2 py-1 text-center">World</a>

           <a [routerLink]="['/book', bookId, 'write']" 
              routerLinkActive="text-purple-600 dark:text-purple-400 font-semibold" 
              class="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition px-2 py-1 text-center">Write</a>
