        ...
        >
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-200">
              Hasil Pencarian untuk: "<span class="text-accent-600 dark:text-accent-400">{{ searchService.searchTerm() }}</span>"
            </h2>
            <button (click)="searchService.closeSearch()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-1 rounded-full focus:outline-none focus:ring-2 focus:ring-accent-500">
                  ...
                  @for (result of results; track result.entityId + result.type) {
                    <li>
                      <a [routerLink]="getLinkPath(result)" 
                         (click)="onResultClick()"
                         class="block p-4 hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-lg transition-colors duration-150 focus:outline-none focus:bg-gray-100 dark:focus:bg-gray-700/50 focus:ring-1 focus:ring-accent-500">
