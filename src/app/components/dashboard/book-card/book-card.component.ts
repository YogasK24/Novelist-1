...
@Component({
  ...
  template: `
    <div
      class="group relative block bg-white dark:bg-gray-800/50 ring-1 ring-gray-200 dark:ring-white/10 rounded-lg shadow-lg hover:ring-purple-500 dark:hover:ring-purple-400/50 transition-all duration-300 overflow-hidden h-full flex flex-col p-4">

      <div class="flex-grow">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1 group-hover:text-purple-600 dark:group-hover:text-purple-300 transition-colors truncate">
          {{ book().title }}
        </h3>
      ...
      <div class="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
        <button (click)="onSetTarget($event)"
          class="text-gray-500 hover:text-green-500 dark:text-gray-400 dark:hover:text-green-400 p-1.5 bg-gray-100/60 dark:bg-gray-900/60 rounded-full focus:outline-none focus:ring-2 focus:ring-green-500"
        ...
        <button (click)="onEdit($event)"
          class="text-gray-500 hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-400 p-1.5 bg-gray-100/60 dark:bg-gray-900/60 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500"
          aria-label="Edit Judul">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"> <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /> </svg>
        </button>
        <button (click)="onDelete($event)"
          class="text-gray-500 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-400 p-1.5 bg-gray-100/60 dark:bg-gray-900/60 rounded-full focus:outline-none focus:ring-2 focus:ring-red-500"
