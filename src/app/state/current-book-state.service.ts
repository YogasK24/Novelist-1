...
export class CurrentBookStateService {
  ...
  readonly isLoading = signal<LoadingState>('initial');
  ...
  private async loadAllData(bookId: number): Promise<void> {
    this.isLoading.set('loading');
    
    try {
        const [bookData, characters, locations, plotEvents, chapters, themes, props] = await Promise.all([
            this.dbService.getBookById(bookId),
            ...
            this.dbService.getThemesByBookId(bookId),
            this.dbService.getPropsByBookId(bookId),
        ]);
        ...
        console.error("Gagal memuat semua data buku:", error);
        this.resetState();
        this.isLoading.set('error');
    }
  }
